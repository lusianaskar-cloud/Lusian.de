...
import { questionCategories } from "@/lib/content/speak";
...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "/speak/questions" },
};
        ...
        <Container className="pb-16 pt-36 lg:pb-24 lg:pt-44">
          ...
          <h1 className="mt-10 max-w-[16ch] font-display text-[clamp(2.3rem,6vw,5rem)] leading-[1.03] tracking-[-0.03em] lg:mt-14">
            <LineReveal
              immediate
              delay={0.15}
              lines={[<span key="1">Asked often enough to answer in public.</span>]}
            />
          </h1>
          <Reveal eager delay={0.28}>
            <p className="mt-9 max-w-xl text-lead text-tone-muted">
              A short list, written properly, rather than forty written quickly. What
              is genuinely specific to your situation is better asked directly.
            </p>
          </Reveal>
        </Container>
      </Section>
                  ...
                  {category.questions.map((item) => (
                    ...
                    >
                      <summary className="flex cursor-pointer list-none items-start justify-between gap-6 py-5 text-[1.0625rem] leading-snug tracking-tight transition-colors duration-500 hover:text-brass">
                        ...
                        <span
                          aria-hidden
                          className="relative mt-2.5 block size-3 shrink-0"
                        >
          ...
          <Reveal wide>
            <div className="flex flex-col gap-6 border-t border-ink/12 pt-10 sm:flex-row sm:items-center sm:justify-between">
              <p className="max-w-md text-[0.9375rem] leading-relaxed text-tone-muted">
                Nothing above is legal, tax or immigration advice, and none of it
                replaces a licensed professional in the relevant jurisdiction.
              </p>
